...
# $Id: pgin.tcl,v 3.23 2004-04-26 01:20:22+00 lbayuk Exp $
#
# Copyright 1998-2004 by ljb (lbayuk@mindspring.com)
...
proc pgtcl::sendmsg {sock type data} {
  set len [expr {[string length $data]+4}]
  puts -nonewline $sock $type[binary format I $len]$data
}
...
proc pgtcl::get_bytes {db n} {
  upvar #0 pgtcl::buf_$db buf pgtcl::bufi_$db bufi
  set result [string range $buf $bufi [expr {$bufi + $n - 1}]]
  incr bufi $n
  return $result
}
...
proc pgtcl::get_rest {db} {
  ...
  set result [string range $buf $bufi end]
  set bufi $bufn
  return $result
}
...
proc pgtcl::get_string {db} {
  ...
  set result [string range $buf $bufi [expr {$end - 1}]]
  set bufi [expr {$end + 1}]
  return $result
}
...
# Format code (text/binary) is not used; Tcl strings are binary safe.
...
proc pg_connect {args} {
  ...
  # Startup packet:
  pgtcl::sendmsg $sock {} [binary format "I a*x a*x a*x a*x a*x a*x x" \
        0x00030000 \
        user $opt(user) database $opt(dbname) options $opt(options)]
...
# Return an error code field value for pg_result -errorField code.
...
proc pgtcl::error_fields {result_name argc code} {
  upvar $result_name result
  variable errnames
...
proc pg_result {res option args} {
  ...
  switch -- $option {
    ...
    -errorField {
      if {$argc != 1} {
        error "-errorField option must be followed by an error code field name"
      }
      return [pgtcl::error_fields result $argc $args]
    }
...
# first; this does not handle unnamed statements.
# Parameters:
#  db          Connection handle
#  stmt        Name of the prepared SQL statement to execute
#  resultinfo  BINARY => Want all results as binary, else as text
#  arginfo     A list describing args: B* => Binary, else Text.
...
proc pg_exec_prepared {db stmt res_formats arg_formats args} {
  ...
  set out [binary format {x a*x S S* S} $stmt $nfcodes $fcodes $nargs]
...
proc pg_copy_write {res line} {
  upvar #0 [pgtcl::checkres $res] result
  pgtcl::sendmsg $result(conn) d "$line\n"
}
